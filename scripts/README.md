...
| Script | What it does |
|---|---|
| `dev-up.sh [--with-frontends] [--seed] [--dummy-data\|--data empty\|catalog\|smoke\|dummy\|full\|exhaustive\|mirror-prod]` | **Bare-run** local env (ADR-0009): Postgres+RabbitMQ in Docker, infra → migrate → all services → (frontends) as host processes. `--with-frontends` starts Storefront `:3000`, Admin `:5200`, and SupplierPortal `:5300`. The light default — never builds images, so it can't OOM the Docker VM. Data profiles: `empty` = as-is/no seed, `catalog` = current sample importer (`--seed`), `smoke` = fast deterministic fixture manifest, `dummy`/`full` = broad demo data via `dev-dummy-data.sh`, `exhaustive` = slower scenario hook, `mirror-prod` = reserved placeholder for a future sanitized prod mirror. |
