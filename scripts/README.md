...
| Script | What it does |
|---|---|
| `dev-up.sh [--fresh] [--with-frontends] [--seed] [--dummy-data\|--data empty\|catalog\|smoke\|dummy\|full\|exhaustive\|mirror-prod]` | **Bare-run** local env (ADR-0009): Postgres+RabbitMQ in Docker, infra → migrate → all services → (frontends) as host processes. `--with-frontends` starts Storefront `:3000`, Admin `:5200`, and SupplierPortal `:5300`. The light default — never builds images, so it can't OOM the Docker VM. `--fresh` first wipes the Postgres data volume (`dev-down --clean`) for a guaranteed-clean start — empty DB → latest migrations → fresh seed on latest-built code; use it whenever local state has drifted (a demo run mutated data, a stale/corrupt DB survived a plain restart). Data profiles: `empty` = as-is/no seed, `catalog` = current sample importer (`--seed`), `smoke` = fast deterministic fixture manifest, `dummy`/`full` = broad demo data via `dev-dummy-data.sh`, `exhaustive` = slower scenario hook, `mirror-prod` = reserved placeholder for a future sanitized prod mirror. |
| `dev-down.sh [--clean\|-v]` | Stops everything `dev-up.sh` started. `--clean`/`-v` also removes the Postgres data volume — plain `down` keeps it, which is why a mutated/corrupt DB can survive restarts; use `--clean` (or `dev-up.sh --fresh`) for a clean slate. |
...
| `dev-dummy-data.sh [--profile smoke\|core\|full\|exhaustive\|mirror-prod]` | Seeds a running dev stack through gateway APIs and writes `.run/dev-dummy-data/fixtures.json` for deterministic Playwright fixtures. `smoke`/`core` import catalog + demo shopper, `full` also attempts tenant/operator data across Entity, Marketing, Pricing, Payments, Fulfillment, Entitlement, and Usage, `exhaustive` is the state-rich scenario hook, and `mirror-prod` is intentionally a no-op placeholder until a sanitized production snapshot flow exists. |
...
| `docker-compose.infra.kafka.yml` / `docker-compose.yml --profile kafka` | Optional Apache Kafka KRaft dev stack for the durable stream lane (ADR-0034), plus Kafka UI on `:8088`. RabbitMQ remains the operational bus and Kafka is not required for normal dev checkout flows. |
