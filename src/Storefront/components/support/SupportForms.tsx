...
// A form section that starts collapsed: click the "+" header to expand and add one; it collapses again
// once the request is created (the ticket action reports ok; the refund action redirects the page).
  ...
  return (
    <div className="rounded-md border border-neutral-200">
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        title={open ? t("collapseForm") : t("expandForm")}
        className="flex w-full items-center gap-2 px-4 py-3 text-left"
      >
        <span
          aria-hidden
          className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full border border-neutral-300 text-lg leading-none text-neutral-600"
        >
          {open ? "−" : "+"}
        </span>
        <span className="font-medium">{title}</span>
        {note && <span className="ml-auto text-sm text-green-700">{note}</span>}
      </button>
...
export function SupportForms({ orderId, refundable, email }: { orderId: string; refundable: RefundableOrder | null; email: string }) {
  ...
  // Collapse the ticket form once it's opened successfully (the refund form redirects, so the page
  // reloads with both collapsed on its own).
  useEffect(() => {
    if (ticketState.ok) setTicketOpen(false);
  }, [ticketState.ok]);

  const refundableLeft = (refundable?.lines ?? []).some((l) => l.quantity > 0);
  ...
  return (
    <div className="space-y-4">
      <Collapsible
        ...
        open={ticketOpen}
        onToggle={() => setTicketOpen((o) => !o)}
        note={ticketState.ok && !ticketOpen ? t("ticketOpened") : undefined}
      >
        <form action={ticketAction} className="space-y-3">
      ...
      <Collapsible title={t("requestRefund")} open={rmaOpen} onToggle={() => setRmaOpen((o) => !o)}>
        <form action={rmaAction} className="space-y-3">
          {rmaState.error && <p className="text-sm text-red-600">{rmaState.error}</p>}
          <input type="hidden" name="orderId" value={orderId} />
          {refundable === null || !refundableLeft ? (
            <p className="text-sm text-neutral-500">{refundable === null ? t("notRefundable") : t("fullyRefunded")}</p>
          ) : (
            <>
                ...
                {refundable.lines.map((line) => (
                  <li key={line.productId} className="flex items-center gap-2 text-sm">
                    <input
                      ...
                      className="w-16 rounded border border-neutral-300 px-2 py-1 disabled:opacity-50"
                    />
