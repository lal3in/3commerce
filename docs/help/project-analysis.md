...
| Payments | Ledger, payment accounts, refunds, saved cards, subscriptions, payouts, Xero mappings. | Append-only balanced ledger; full dispute/chargeback lifecycle (created→won/lost with terminal chargeback + void record); direct-debit mandates (ACH/SEPA/BACS/BECS/ACSS by currency); rotation-safe provider webhooks re-registered on account activate/suspend. | Live Xero/Stripe credentials remain launch gates. Off-session subscription renewals still charge a generic card kind — wiring renewals to the customer's stored mandate/token is a tracked follow-up. |
...
| Workflow | Scheduled job run history and scheduling policy. | Typed jobs, run records, persistent scheduler path. | Configurable Quartz misfire policy + bounded job retry are in place (`Quartz:MisfirePolicy` / `MaxJobRetries` / `RetryDelaySeconds`), and Mission Control shows per-run **duration**; deeper dashboard analytics remain incremental. |
...
| Usage | Metered usage balances and overage billing events. | Incremental balances; idempotent recording; overage gate. | Subscription billing + the dispute/chargeback lifecycle are now hardened; the remaining recurring-money gap is metered usage-overage **period reset / auto-billing** and its accounting journals, which still need a production-grade closing flow. |
