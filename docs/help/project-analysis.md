...
| Payments | Ledger, payment accounts, refunds, saved cards, subscriptions, payouts, Xero mappings. | Append-only balanced ledger; full dispute/chargeback lifecycle (created→won/lost with terminal chargeback + void record); direct-debit mandates (ACH/SEPA/BACS/BECS/ACSS by currency); rotation-safe provider webhooks; subscriptions **auto-renew per storefront** on a daily timer, charging the customer's stored instrument (card/mandate) **off-session**. | Live Xero/Stripe credentials remain launch gates. One tracked follow-up: renewal revenue still posts to the shared `revenue.sales`/`tax` accounts — routing it to the storefront's **own** ledger accounts (now that subscriptions carry `StorefrontId`) is `pay_renew_storefront_ledger`. |
...
| Support | Tickets and RMA saga. | Server-derived RMA amount and refund saga reuse. | Admins can now open **per-line partial** RMA refunds (`POST /api/support/admin/rmas` with a `lines` selection) alongside whole-order returns; restock granularity covers partial return stock. |
| Entity | Parties, identifiers, addresses, supplier onboarding/change requests. | Master-data foundation with maker-checker and full RLS coverage. | FORCE row-level security now covers all Entity child tables — own-`TenantId` tables directly, and children keyed only by `EntityId` via a parent-join to `Entities.TenantId` (ADR-0024). |
...
| Workflow | Central scheduled-job registry + run history; cross-service Scheduled-Job Manager. | Typed jobs, run records, persistent scheduler path, live per-service registry. | A full **Scheduled-Job Manager** now surfaces every service's jobs in Mission Control with **run-now / pause-resume / edit-cron** (persisted overrides that survive restart), on top of configurable Quartz misfire/retry (`Quartz:MisfirePolicy` / `MaxJobRetries` / `RetryDelaySeconds`) and per-run duration. |
| Entitlement | Digital/service-line access on confirmed orders. | Digital lines do not create physical shipments. | Storefront now has a self-service **my-access** page (`/account/access`) listing entitlement type/status/granted/expiry; richer UX remains incremental. |
| Usage | Metered usage balances, overage billing, and prepaid auto-load. | Incremental balances; idempotent recording; overage gate; period-close + prepaid auto-load. | Metered auto-billing is now closed: a daily job **closes/rolls billing periods** (billing unbilled overage), and customers can opt into **prepaid auto-load** — usage tops up their credit off-session the moment it crosses their threshold, with a daily safety-net sweep. Off-session settlement against the stored instrument is the same refinement shared with arrears overage. |
...
| RLS/tenant isolation rollout unevenness | Identity + Entity (incl. all Entity child tables) use FORCE RLS; some services still use deliberate app-level tenant filtering. | Tenant middleware + non-superuser RLS tests exist (Entity child-table isolation now covered); extend the same posture to the remaining app-level services before production. |
