...
| [Supplier functionality & management](./supplier-functionality.md) | The supplier model end-to-end: Entity master-data lifecycle (Draft→…→Active), the Admin **Suppliers** page (details, one-click **Approve**, per-variant **supplier cost**), Supplier Portal self-service under the **approval lock** (maker-checker change requests), **Offers** (cost vs storefront-scoped active-window price, charged == shown), **approval-gated availability** (Decision A), and forthcoming warehouse/collect-at-warehouse. ADRs 0027/0028/0047/0048. |
