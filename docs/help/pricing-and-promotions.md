...
- A **stale-hold sweep** (reservations older than 45 minutes with no checkout attempt and no order)
  ...
  nothing at all.
- **Subscriptions: introductory or permanent?** ([ADR-0057](../adr/0057-subscription-renewal-price-and-introductory-promotions.md))
  `Promotion.AppliesToRenewals` decides. Unset (the default) is an **introductory** offer — the first
  period is discounted and renewals charge the list price. Set, the discount rides every renewal. When
  promotions stack, only the flagged half rides. The **storefront-wide discount never rides a renewal**:
  it is a sale the store is running today, not a term of the subscription. The renewal-carrying share is
...
- **A refund never gives a redemption back** ([ADR-0056](../adr/0056-a-refunded-order-keeps-its-redemption.md)).
  ...
  was taken up) and not the revenue. Releasing would let a shopper reset a one-shot code by returning the
  order. Release stays correct only where no sale ever happened — failed payment, cancel, expiry, and the
  stale sweep. The rule is enforced by the `Status = 'Reserved'` guard inside `ReleaseAsync`, so a refund
  path that calls it changes nothing.
