...
| `Scheduling:Enabled` | Payments, Usage, Marketing, Workflow | `true` | Quartz schedulers + the Scheduled-Job Manager control surface (`/admin/jobs/*`). Jobs: `daily-journal` (Payments), `subscription-auto-renew` (Payments, every 15 min — renews due subs per storefront at that storefront's configured daily time, default 12:00:00, set via `PUT /api/payments/admin/billing-schedules/{storefrontId}`), `usage-period-close` (Usage — daily period close + prepaid auto-load safety-net sweep), `scheduled-publish` (Marketing). Operators can run-now / pause / edit each job's cron from Mission Control (overrides persist). Integration tests turn it off. |
