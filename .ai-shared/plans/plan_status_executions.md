...
Last Modified Date-Time: 2026-06-24 (mt4_1 done; supply-type comparison → ADR-0028 supply seam inserted as mt4_1b ahead of mt4_2, mt4_2 widened (offer-reservations + inventory_movements + single stock owner), mt4_4b dropship added, Phase 7 digital-supply/billing added)
...
| mt4_1b | Supply seam — Offers + supplier_type + fulfilment_type + price-off-variant | Phase 4 shipping/inventory/fulfillment | pending | .ai-shared/plans/multi-tenant-platform-expansion-phase-4-shipping-inventory-fulfillment.md | ADR-0028. RUN NEXT (before mt4_2). First-class Offer/product_supply_profile (multi-supplier); add SupplierType (Entity) + product_type (Catalog); unify the two FulfillmentSource enums + bus string into one typed FulfilmentType; move price to the Offer (pricing_model=one_time now); order line gains billing_mode. mt4_1 InventoryItem becomes the warehouse strategy. Do now while ~6 call sites. |
| mt4_2 | Hybrid inventory reservations + movement ledger + single stock owner | Phase 4 shipping/inventory/fulfillment | pending | .ai-shared/plans/multi-tenant-platform-expansion-phase-4-shipping-inventory-fulfillment.md | Hard reserve during checkout/payment saga AGAINST THE OFFER (warehouse→reserved; dropship→soft; digital→no-op). Add inventory_movements ledger (reservation IS a movement; seeds audit mt6_1). Collapse duplicate stock: Fulfillment InventoryItem is the single owner, Catalog Variant.StockQuantity becomes a projected read model. Idempotent/balanced. |
