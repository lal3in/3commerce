...
| mt2_7 | Admin/CLI entity and supplier management | Phase 2 entity/supplier | in_progress | .ai-shared/plans/multi-tenant-platform-expansion-phase-2-entity-supplier.md | 7a/7b supplier change-request lifecycle DONE (SupplierChangeRequest aggregate w/ maker-checker + 8 tests; service + DbSet + additive migration; admin open/list/approve/reject endpoints, acting principal from sub claim). PENDING: customer entity link/de-link, Admin entities/suppliers page + tenant selector, CLI entity/supplier commands |
